    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        for (v1, probe_view) in enumerate(view_list):
            ...
            for (v2, gallery_view) in enumerate(view_list):
                ...
                gallery_y = label[gseq_mask]
                gallery_x = feature[gseq_mask, :]
                probe_type = np.array(seq_type)[pseq_mask]
                gallery_type = np.array(seq_type)[gseq_mask]
                dist = cuda_dist(probe_x, gallery_x, metric)
                num_rank = 2
                idx = dist.topk(num_rank, largest=False)[1].cpu().numpy()
                ### show the rank-1 failure cases()
                # acc[type_][v1, v2, :] = np.round(np.sum(np.cumsum(np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:num_rank]], 1) > 0,
                #                                      0) * 100 / dist.shape[0], 2)
                gallery_type_2 = gallery_type[idx[:, 0:num_rank]]
                match_rst = np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:1]]
     
                rank2_match = np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 1:2]]
                for i in range(len(match_rst)):
                    if not match_rst[i] and rank2_match[i]:
                        print("gt:", probe_y[i], probe_type[i], view_list[v1], "pred: rank-1", view_list[v2], match_rst[i], gallery_y[idx[:, 0:1]][i], gallery_type_2[i][0], "pred: rank-2",rank2_match[i], gallery_y[idx[:, 1:2]][i], gallery_type_2[i][1])
